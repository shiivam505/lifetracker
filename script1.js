document.addEventListener('DOMContentLoaded', () => {
    ...
    const renderMonthlyHabitRecords = () => {
        ...
        if (habitData[currentMonth]) {
            ...
            // Combine default and custom habits for display
            const allHabitNames = [...new Set([...defaultHabits.map(h => h.name), ...customHabits.map(h => h.name)])];
    ...
    const drawHabitGraph = () => {
        ...
        // Get all unique habit names (default + custom)
        const allHabitNames = [...new Set([...defaultHabits.map(h => h.name), ...customHabits.map(h => h.name)])];
        ...
        const barWidth = (svgWidth - 2 * padding) / (allHabitNames.length * 1.5); // Adjusted for spacing
    ...
    const renderAllHabitsList = () => {
        ...
        // Display default habits (not deletable from this list)
        defaultHabits.forEach(habit => {
            const li = document.createElement('li');
            li.innerHTML = `<i class="${habit.icon}"></i> <span class="habit-name-display">${habit.name}</span>`;
            li.classList.add('default-habit-item'); // Add class to style default habits
            allHabitsList.appendChild(li);
        });

        // Display custom habits (deletable)
        customHabits.forEach((habit, index) => {
            const li = document.createElement('li');
            li.innerHTML = `
                <i class="${habit.icon}"></i>
                <span class="habit-name-display">${habit.name}</span>
                <button class="delete-habit-item-btn" data-index="${index}"><i class="fas fa-trash-alt"></i></button>
            `;
            allHabitsList.appendChild(li);
        });
        ...
        allHabitsList.querySelectorAll('.delete-habit-item-btn').forEach(button => {
            button.addEventListener('click', (event) => {
                ...
                if (!isNaN(indexToDelete)) {
                    customHabits.splice(indexToDelete, 1); // Remove from array
                    saveCustomHabits(); // Save updated array
...
loadEvents();
    ...
    const renderTransactionHistory = () => {
        ...
        sortedTransactions.forEach(trans => {
            if (trans.accountId === currentAccount.id) { // Only show transactions for current account
                ...
                // Format date for history: DD MMM, HH:MM (if time is available) or DD MMM YYYY (if only date)
    ...
    showFinanceGraphBtn.addEventListener('click', () => {
        ...
        if (!expenditureGraphSection.classList.contains('hidden')) {
            drawExpenditureGraph(); // Draw graph only when visible
        }
    });
